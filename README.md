...
two sentences are broken. The default behavior — rewrite the whole thing for
consistency — is exactly the problem.
...
because improving things is what a model defaults to when given latitude and
...
confirmation before writing a single output sentence — the same
...
The full profile template, the extraction algorithm for each dimension, a
genre-compatibility guide (a profile from a casual newsletter doesn't carry
over to a technical spec), and worked examples of both modes are in
